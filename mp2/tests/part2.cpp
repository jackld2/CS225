...
TEST_CASE("Base Only Sheet", "[weight=5][part=2]") {
  Image alma; alma.readFromFile("tests/alma.png");

  StickerSheet sheet(alma, 5);
  REQUIRE(sheet.get_max() == 5);
  REQUIRE(sheet.get_size() == 0);
  Image expected;
  expected.readFromFile("tests/expected-base.png");
  REQUIRE( sheet.render() == expected );
}

TEST_CASE("Sticker is added", "[weight=5][part=2]") {
    Image alma; alma.readFromFile("tests/alma.png");
    Image i;    i.readFromFile("tests/i.png");
    StickerSheet sheet(alma, 5);
    sheet.addSticker(i, 20, 200);
    REQUIRE(sheet.get_size() == 1);
}
