...
class KDTree
{
  ...
  public:
    ...
    //Point<Dim> select(vector<Point<Dim>>& points, int left, int right, int k, int dim);
  ...
  private:
     ...
     */
     ...
     int partition(vector<Point<Dim>>& points, int left, int right, int pivotIndex, int dim);
     Point<Dim> select(vector<Point<Dim>>& points, int left, int right, int k, int dim);
     KDTreeNode* construct(vector<Point<Dim>>& points, int left, int right, int dim);

     void destroy(KDTreeNode* node);
     KDTreeNode* copy(const KDTreeNode* original);
     double pointDistance(Point<Dim>& a, Point<Dim>& b) const;

     //Point<Dim> nearest(const Point<Dim>& query, Point<Dim> currentBest, KDTreeNode* node, int dim ) const;
     void nearestNode(Point<Dim>& query, Point<Dim>& currentBest, KDTreeNode* node, int dim ) const;
};
